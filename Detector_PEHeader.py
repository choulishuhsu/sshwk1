...
PEHeaders = ['Magic', 'MajorLinkerVersion', 'MinorLinkerVersion',
...
def get_peheader_features(path):
    ...
    for item in pe.OPTIONAL_HEADER.__keys__:
        for tem in item:
            peheader_features[tem] = peheader.__getattribute__(item[0])
...
def train_detector(benign_path,malicious_path):
    ...
    pickle.dump(classifieropen("saved_detector.pkl","wb+"))
